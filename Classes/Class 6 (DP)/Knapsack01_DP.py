...
def knapsack01(capacity, current_product, products):
    if capacity == 0 or current_product >= len(products):
        return 0
    elif dpTable[current_product][capacity] != None:
        return dpTable[current_product][capacity]
    elif products[current_product].weight > capacity:
        result = knapsack01(capacity, current_product + 1, products)
        dpTable[current_product][capacity] = result
        return result
    else:
